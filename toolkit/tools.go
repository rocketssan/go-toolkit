...
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data any) error {
	...
	if err != nil {
		...
		case strings.HasPrefix(err.Error(), "json: unknown field"):
			...
			return fmt.Errorf("body contains enknown key %s", fieldName)
...
// ErrorJSON takes an error, and optianally a status code, and generates and sends a json error message.
...
// PushJSONToRemote sends aribrary data to some URL as JSON, and returns the response, status code, and error, if any.
// The final parameter, client, is optional. If none is specified, we use the standard http.Client.
func (t *Tools) PushJSONToRemote(uri string, method string, data any, client ...*http.Client) (*http.Response, int, error) {
	switch method {
	...
	case http.MethodPatch:
		break
	default:
		return nil, 0, fmt.Errorf("method not allowed. method: %s", method)
	}
	// create json
